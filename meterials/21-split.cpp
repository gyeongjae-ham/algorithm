...
// * C++에서는 불행하게도 문자열을 기반으로 split 함수를 지원하지 않는다 따라서 다음과 같이 구현해야 한다
...
vector<string> split(string input, string delimiter) {
  ...
  long long pos = 0;
  string token = "";
  while ((pos = input.find(delimiter)) != string::npos) { // pos에 delimiter의 인덱스 값이 들어간다
    token = input.substr(0, pos); // token에 문자열 처음부터 인덱스값 전까지 담고
    ret.push_back(token); // ret에 해당 값을 담아준다
    input.erase(0, pos + delimiter.length()); // 원래 문자열에서 token과 delimiter(예를 들면 빈칸)까지 모두 삭제해준다
  }
  ret.push_back(input);
  return ret;
}
